...
echo '<article class="comment-form">';
    ...
    $defaults = array(
            ...
            'logged_in_as'          =>  '<p class="comment-notes">' . esc_html__("Logged in as",'newscrunch'  ).' '.'<a href="'. esc_url(admin_url( 'profile.php' )).'">'.$user_identity.'</a>'. '<a href="'. esc_url(wp_logout_url( get_permalink() )).'" title="'.esc_html__('Log out from this Account','newscrunch' ).'">'.' '.esc_html__("Log out",'newscrunch' ).'?</a>' . '</p>',
            'id_submit'             =>  'submit',
